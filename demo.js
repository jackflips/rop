...
function findPath() {
    findPathHelper(0, 0, []);
    for (i=possiblePaths.length-1; i>0; i--) {
        if (pathStillValid(possiblePaths[i])) {
...
function selectNewTargetsForNodes() {
    ...
    for (node in nodes) {
        ...
        if (theNode.speed > 30) {
            theNode.speed = 30;
        } else if (theNode.speed < -30) {
            theNode.speed = -30;
        }
        if (theNode.currentLocation[0] > 480 || theNode.currentLocation[0] < 20 || theNode.currentLocation[1] > 480 || theNode.currentLocation[1] < 20) {
            theNode.angle += 180;
        }
    }
}
...
function updateNodes() {
    for (node in nodes) {
        ...
        theNode.currentLocation = [(theNode.currentLocation[0] + x).mod(500), (theNode.currentLocation[1] + y).mod(500)];
...
function draw() {
    ...
    for (i=0; i<nodes.length; i++) {
        ...
        } else if (currentPath.indexOf(i) != -1) {
        ...
        if (currentPath.indexOf(i) != -1 && i > 0) {
...
$(function() {
    ...
    for (i=0; i<numNodes; i++) {
        if (i == 0) {
            nodes.push(new Node([getRandInRange(0, 500), getRandInRange(0, 500)], 0, 1, true));
        }
        nodes.push(new Node([getRandInRange(0, 500), getRandInRange(0, 500)], 0, 1, false));
    }
    var canvas = document.getElementById('canvas');
